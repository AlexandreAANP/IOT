...
class MQTTClient():
    instance = None
    def __init__(self,name, hostname, port, subscribeList) -> None:
        ...
        self.bSendStatus = True
        self.threadStatus = threading.Thread(target=self.sendStatus)
    ...
    def sendStatus(self):
        ...
        while self.bSendStatus:
            try:
                with open("environmentLogs", "r") as f:
            ...
            self.client.publish(self.name, payload=json.dumps(status),qos=0)
            time.sleep(5)

    @classmethod
    def getMQTTClient(cls,name="",subscribeList:list = [], hostname="localhost", port=1883 ):
        if cls.instance == None:
            cls.instance = cls(name,hostname,port,tuple(subscribeList))
        return cls.instance
    ...
    def on_message(self,client, userdata, msg):
        ...
        try:
            if content["id"] == "api" and content["action"] =="setVideo":
                with open("videoToRead","w") as f:
                    f.write(content["video"])
        except Exception as e:
            return print(e)
        print(content)
